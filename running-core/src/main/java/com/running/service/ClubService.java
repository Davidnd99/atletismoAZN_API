...
public class ClubService {
    ...
    /**
     * Actualiza datos del club. Si se cambia el nombre, no se permite duplicar.
     */
    @Transactional
    public ClubDto updateClub(Long clubId, ClubDto data) {
        ...
        if (data.getName() != null) {
            ...
            if (!newName.equalsIgnoreCase(club.getName())) {
                if (clubRepository.existsByNameIgnoreCaseAndIdNot(newName, club.getId())) {
        ...
        try {
            ...
            throw new ResponseStatusException(HttpStatus.CONFLICT, "Ya existe un club con ese nombre", ex);
        }
    }
    ...
    public AdminClubDto updateClubManager(Long clubId, String managerEmail) {
        ...
        if (!isClubAdmin && !isAdmin(newManager)) {
            throw new ResponseStatusException(HttpStatus.FORBIDDEN, "El usuario debe ser 'club-administrator' o 'admin'");
        }
